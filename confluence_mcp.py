...
def search_pages(
    query: str, limit: int = 10, spaces_filter: Optional[str] = None
) -> List[Dict]:
    ...
    for page in MOCK_PAGES:
        ...
        if (
            ...
            or any(query_lower in label.lower() for label in page["labels"])
        ):
...
async def search(
    ...
    """
    logger.info(
        f"Searching Confluence with query: {query}, limit: {limit}, spaces_filter: {spaces_filter}"
    )
    ...
    if query and not any(
        ...
        try:
            ...
            pages = search_pages(
                original_query, limit=limit, spaces_filter=spaces_filter
            )
...
async def get_page(page_id: str) -> str:
    ...
    if not page:
        return json.dumps(
            {"error": f"Page with ID {page_id} not found"}, indent=2, ensure_ascii=False
        )
...
# Custom middleware to log all requests and headers
...
if __name__ == "__main__":
    ...
    # Log the SSE endpoint and messages endpoint
